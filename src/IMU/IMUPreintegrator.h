...
public:
    ...
    {
        Matrix3d Jr = Matrix3d::Identity();   //gzh 2019-8-19 fix as the preint-formula
        Matrix3d Jr0 = Matrix3d::Identity();
        double theta = w.norm();
        if(theta<0.00001)
        {
            return Jr0;// = Matrix3d::Identity();
        }
        else
        {
            Vector3d k = w.normalized();  // k - unit direction vector of w
            Matrix3d K = skew(k);         // skew->[SO3::hat( k )]   Vector3d k-> Matrix3d K

            Jr0 =   Matrix3d::Identity()    //
                    - (1-cos(theta))/theta*K
                    + (1-sin(theta)/theta)*K*K;

            Jr =   Matrix3d::Identity()
                   - K*(1-cos(theta))/(theta*theta)
                   + K*K*(theta-sin(theta))/(theta*theta*theta);
            //cout<<"Jr"<<Jr<<endl; cout<<"Jr0"<<Jr0<<endl;
        }
        return Jr0;
    }
