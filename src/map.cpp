...
void Map::addKeyframe(FramePtr new_keyframe)
{
    if(keyframes_.size() >0 ){
        new_keyframe.get()->kf_id_ = keyframes_.back().get()->kf_id_+1;  //get kf id from prev Keyframe + 1
        keyframes_.back()->SetNextKeyFrame( new_keyframe.get() );        //update the prev Keyframe next Keyframe
        new_keyframe->SetPrevKeyFrame( keyframes_.back().get() );        //update the current key frame prev KeyFrame
    }
    else
        new_keyframe.get()->kf_id_ = 1;

    keyframes_.push_back(new_keyframe);

    if(keyframes_.back().get()->kf_id_ > MaxKFid)
        MaxKFid = keyframes_.back().get()->kf_id_;
    cout<<"add keyframe to map ; max kf id is : "<< MaxKFid<<endl;
}
...
{
  boost::unique_lock<boost::mutex> lock(mut_);
  PointCandidateList::iterator it=candidates_.begin();   //it =  pair<Point*, Feature*>
